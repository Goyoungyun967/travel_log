...
const InquiryView = () => {
    ...
    useEffect(()=>{
        axios.get(`${backServer}/inquiry/${inquiryNo}`).then((res)=>{
            if(res.data){
                setInquiry(res.data);
            }
        }).catch((err)=>{
    ...
    const fileDown = (file) => {
        axios.get(`${backServer}/inquiry/file/${file.filepath}`,{
            responseType : "blob",
        }).then((res)=>{
    ...
    const writeInquiryReply = () => {
        axios.post(`${backServer}/inquiry/inquiryReply`,{inquiryNo : inquiry.inquiryNo, inquiryReplyContent}).then((res)=>{
            if(res.data > 0){
                Swal.fire({
                    title : "문의 답변 완료",
                    text : "답변이 완료 되었습니다.",
        ...
        <div className="inquiry-view-wrap">
            <table>
                <tbody>
                    ...
                    <tr>
                        <th>첨부파일 목록</th>
                        <td>{inquiry.inquiryFileList.map((file,index)=>{
                            return (
                                <div key={`inquiry-file-${index}`} className="inquiry-file-wrap">
                    ...
                    <tr>
                        <td colSpan={2} className="inquiry-content" dangerouslySetInnerHTML={{__html : inquiry.inquiryContent}}></td>
                    </tr>
                </tbody>
            </table>
